...
models = model_loader.preload_models_from_standard_weights(model_file, DEVICE, in_channels=in_channels,
                                                           out_channels=out_channels, image_size=(image_size))
...
logger.info('resume path:' + str(resume_path))
...
logger.info('clip input prompt text(positive):' + str(prompt))
logger.info('clip input prompt text(negative):' + uncond_prompt)
logger.info('selected channels:' + str(selected_channels))
logger.info('selected channels (rou):' + str(rou))
## IMAGE TO IMAGE

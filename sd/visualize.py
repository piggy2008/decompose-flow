...
def compare_every_channel(x, dark_channel_prior):
    # FFT
    dark_norm = (dark_channel_prior-np.min(dark_channel_prior))/(np.max(dark_channel_prior)-np.min(dark_channel_prior))
    b, c, h, w = x.shape
    feature_maps = x.data.cpu().numpy()
    compare_dict = {}
    for i in range(0, c):
        data = feature_maps[0, i]
        norm = (data-np.min(data))/(np.max(data)-np.min(data))
        bhatt_dist = calculate_correlation_coefficient(norm, dark_norm)
        # print('unit: {}, value: {}'.format(i, bhatt_dist))
        compare_dict[i] = bhatt_dist
    compare_dict_sort = {k: v for k, v in sorted(compare_dict.items(), key=lambda item: item[1])}
    return compare_dict_sort
...
def turn_off_tree_units(data, model):
    ...
    # print('scale:', ((1.8 - 1) * hidden_mean + 1).shape)
...
if __name__ == '__main__':
    ...
    check_layer = 'blocks.0'
    # units = range(0, 1)

    # dark channel extraction
    # result_image = result_image.resize((64, 64))
    dark_result_image = cal_Dark_Channel(np.array(result_image) / 255, width=5)
    ...
    # 3: visualize output if one unit is changed
    test_all_units = 0
    if test_all_units == 0:
        dis_dict = {}
        count = 0
        ...
        print(final)
        print(f"qualified unit num:", count)
        ...
        for check_layer in check_layers:
            ...
            # dark_modifed_image = cal_Dark_Channel(output_image / 255)
            scores = compare_every_channel(acts, dark_result_image)
            scores_pos = {}
            scores_neg = {}
            print('layer in', check_layer, 'all channel scores ranking:', scores)
            for k, v in scores.items():
                if v > 0.8:
                    scores_pos[k] = v
                elif v < -0.8:
                    scores_neg[k] = v
            print('layer in', check_layer, 'positive-related feature map index:', scores_pos)
            print('layer in', check_layer, 'negative-related feature map index:', scores_neg)
    ...
    elif test_all_units == 3:
        # units = [378, 345, 339, 333, 275, 242, 194, 164, 144, 92] [367]
        # set_units = [i]
        units = [378, 345]
        set_units = units
        check_layers = ['blocks.0']
        ...
        dark_result_image_modified = cal_Dark_Channel(np.array(output_image) / 255, width=5)
        # corr_coeff = calculate_ssim(dark_result_image, output_image)
        # print(f"SSIM: {corr_coeff}")
        # Correlation Coefficient: 0.9894616106237757 --- 0.5679176808407624
        # SSIM  0.589 --- 0.13410
        plt.subplot(2, 2, 1)
        ...
        plt.subplot(2, 2, 4)
        plt.imshow(dark_result_image_modified)
        # plt.savefig('../data/' + image_name + '_compare.png')
        # plt.subplot(3, 2, 5)
        # plt.imshow(np.log(1 + magnitude[0, 378]))
        # plt.subplot(3, 2, 6)
        # plt.imshow(np.log(1 + magnitude[0, 174]))
        # plt.show()
        Image.fromarray(output_image).save('../data/' + image_name + '_compare_dcp.png')
# value: 0.5

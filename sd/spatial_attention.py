...
def Fourier_filter(x, threshold, selected_channels):
    ...
    x_freq = x_freq * mask.to(x_freq.device)
...
class CrossAttention2(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        h = self.heads
        # q_selected_input = x[:, :, self.selected_channels]
        ...
        # out[:, :, self.selected_channels] = 0.5 * out_selected + 0.5 * out[:, :, self.selected_channels]
        return out
